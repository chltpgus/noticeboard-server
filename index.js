...
const corsOptions = {
    //클라이언트가 cors문제 없이 들어 올 수 있게 권한을 줌
...
setInterval(() => {
    //MYSQL이 방치되면 꺼지는 것을 막기위해 주소를 계속 보내줌
...
server.get('/signup', (req, res) => {
    ...
    connection.query('SELECT * FROM signup', function (err, row) {
        //signup 데이터베이스 SELECT
        res.json(row); //        signup 정보를 서버에 전송
    });
});

server.get('/written', (req, res) => {
    // get요청이 오면

    connection.query('SELECT * FROM written', function (err, row) {
        //written 데이터베이스 SELECT
        res.json(row); //  서버에 값들을 전송한다.
    });
});

server.get('/signup/email=:email', (req, res) => {
    // 회원 정보 email get 요청이 오면
...
server.get('/signup/nickname=:nickname', (req, res) => {
    // 회원 정보 nickname get 요청이 오면
...
server.post('/signup', (req, res) => {
    ...
    connection.query('SELECT * FROM signup', function (err, row) {
        ...
        let email01 = signup1.email,
            password01 = signup1.password,
            nickname01 = signup1.nickname; //변수에 POST 전송으로 온 값을 저장
...
server.post('/written', (req, res) => {
    // post전송이 오면

    connection.query('SELECT * FROM written', function (err, row) {
        written1 = req.body; //POST로 전송된 jSON written1에 저장
        res.json(row); // 서버에 json으로 보내기
        let nickname01 = written1.nickname,
            title01 = written1.title,
            data01 = written1.date,
            maintext01 = written1.maintext;
        let sql =
            "INSERT into written( nickname, title, date, maintext)values('" +
            nickname01 +
            "','" +
            title01 +
            "','" +
...
server.post('/written/delete', (req, res) => {
    // post 전송이 오면
    connection.query('SELECT * FROM written', function (err, row) {
        ...
        let nickname01 = written1.nickname,
            ...
            maintext01 = written1.maintext;
        let sql =
            "DELETE from written where nickname = '" +
            nickname01 +
            "'and title = '" +
            title01 +
            "'and date = '" +
...
server.post('/written/update', (req, res) => {
    // post 전송이 오면
    ...
    connection.query('SELECT * FROM written', function (err, row) {
        ...
        let nickname01 = written1.nickname,
            ...
            data01 = written1.date,
            maintext01 = written1.maintext,
            title02 = written1.title2,
            maintext02 = written1.maintext2;
        let UPDATE =
            "UPDATE written SET title = '" +
            title02 +
            "', maintext = '" +
            maintext02 +
            "' WHERE nickname = '" +
